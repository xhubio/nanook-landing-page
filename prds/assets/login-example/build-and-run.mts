...
const rows: Row[] = [
  ...
  row('valid',    'gen:1:faker:internet.email', '',            { 0:'x', 4:'a', 5:'a', 6:'x', 7:'x' }),
  row('empty',    '',                            'min(1)',      { 1:'x' }),
  row('no @',     'torsten.link',                'format',      { 2:'x' }),
  row('too long', 'gen::len:255',                'max(254) RFC 5321', { 3:'x' }),

  sec('password', 'FieldSubSection'),
  row('valid',     'gen:1:faker:internet.password', '',        { 0:'x', 1:'a', 2:'a', 3:'a', 6:'x', 7:'x' }),
  row('empty',     '',                              'min(1)',   { 4:'x' }),
  row('too short', 'ab',                            'min(8)',   { 5:'x' }),
  ...
  row('failed-attempt counter rises', '',
      'SOLL: only for a real account with a wrong password — not for an unknown one',
      { 5:'x', 0:'e', 6:'e' }),
  ['<END>', '', '', '', '', ...Array(n).fill('')],
]
