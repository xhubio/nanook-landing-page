...
# aendern (brutalist.css, theme.js). Nach JEDER Aenderung an einer der beiden
...
set -e
cd "$(dirname "$0")/.."
CSS_HASH=$(shasum css/brutalist.css | cut -c1-8)
JS_HASH=$(shasum js/theme.js | cut -c1-8)
find . -name "*.html" -not -path "./.git/*" -print0 | xargs -0 perl -pi -e "
  s{/css/brutalist\.css(\?v=[0-9a-f]+)?}{/css/brutalist.css?v=$CSS_HASH}g;
  s{/js/theme\.js(\?v=[0-9a-f]+)?}{/js/theme.js?v=$JS_HASH}g;
"
echo "brutalist.css -> ?v=$CSS_HASH; theme.js -> ?v=$JS_HASH"
